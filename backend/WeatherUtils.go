...
func Current(county string) []float64 {
	...
	req.Header.Add("x-rapidapi-key", "YOUR_KEY_HERE")
	req.Header.Add("x-rapidapi-host", "community-open-weather-map.p.rapidapi.com")
...
func main1() {
	...
	//	start := time.Date(2020, 12, 10, 11, 45, 0, 0, time.UTC)
	//	end := time.Date(2020, 12, 12, 0, 0, 0, 0, time.UTC)
	//
	//	wis := RandomHistoricalWeather(start, end)
	//
	//	for _, v := range wis {
	//		fmt.Printf("%+v\n",v)
	//	}
	...
	wis := Forecast("tompkins")
	for i := 0; i < len(wis); i++ {
		fmt.Printf("%+v\n", wis[i])
	}

	//fmt.Println( wis[4].dt.Sub(wis[3].dt) )
